	...
	const fetchSuggestions = async (value, isPickup = true) => {
		if (value.length > 2) {
			...
			if (isPickup) {
				...
				setHighlightIndexPickup(-1); // Reset highlight index
			} else {
				setDestinationSuggestions(suggestions);
				setHighlightIndexDest(-1); // Reset highlight index
			}
		} else {
									...
									{destinationSuggestions.length > 0 && (
										...
										>
											{destinationSuggestions.map((suggestion, index) => (
												<li
													id={`destination-dropdown-${index}`} // ✅ Unique ID for scrolling
													key={suggestion.id}
													onClick={() =>
														handleSelectAddress(suggestion.id, false)
													}
													className={`px-3 sm:px-4 py-2 hover:bg-gray-100 cursor-pointer text-xs sm:text-sm ${
														highlightIndexDest === index ? 'bg-gray-200' : ''
													}`}
												>
													{suggestion.label}
												</li>
